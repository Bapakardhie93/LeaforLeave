...
struct OnboardingView: View {
    ...
    var body: some View { VStack(spacing: 24) { Spacer(); Image(systemName: pages[page].2).font(.system(size: 50)).foregroundStyle(LeafColors.accent); Text(pages[page].0).font(.largeTitle.bold()); Text(pages[page].1).foregroundStyle(.secondary).multilineTextAlignment(.center).frame(maxWidth: 430); Spacer(); HStack { Button("Skip") { finish() }; Spacer(); Button(page == pages.count - 1 ? "Finish" : "Continue") { if page == pages.count - 1 { finish() } else { page += 1 } }.buttonStyle(.borderedProminent) } }.padding(36).frame(width: 620, height: 440) }
